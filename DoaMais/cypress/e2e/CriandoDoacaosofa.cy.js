describe('Teste de adicionar item para doação', () => {
    it('Deve permitir ao usuário adicionar um novo item para doação', () => {
      ...
      cy.get('input[type="file"]').attachFile( 'sofa_vermelho.jpeg'); 
  
      // Preencher os campos do formulário
      cy.get('input#item_name').type('Sofá Vermelho');
      cy.get('select#category').select('furniture');
      cy.get('select#condition').select('new');
      cy.get('input#location').type('Recife, PE');
      cy.get('textarea#description').type('Sofá vermelho de dois lugares');
